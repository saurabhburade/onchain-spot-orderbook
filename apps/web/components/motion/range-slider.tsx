...
export function RangeSlider({ showTicks = true, className, ...options }: RangeSliderProps) {
  ...
  return (
    <div
      {...trackProps}
      className={cn(
        "relative flex h-8 w-full touch-none items-center overflow-hidden rounded-lg bg-muted",
        TOUCH_GESTURE_CLASS,
    ...
    >
      ...
      <motion.div
        ...
        className="absolute left-0 top-1/2 h-5 w-1 rounded-full bg-foreground outline-none ring-inset ring-foreground/30 focus-visible:ring-4"
        style={{ x: thumbX, y: "-50%" }}
      />
    </div>
  );
