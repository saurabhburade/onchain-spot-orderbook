...
function ToastTitle({ className, ...props }: ToastPrimitive.Title.Props) {
  return <ToastPrimitive.Title className={cn("text-sm font-medium", className)} data-slot="toast-title" {...props} />;
}
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "error") icon = <OctagonXIcon aria-hidden="true" className="text-destructive" />;
  if (type === "loading") icon = <Loader2Icon aria-hidden="true" className="animate-spin" />;
  if (!icon) return null;

  return (
    <span className="shrink-0 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4" data-slot="toast-icon">
      {icon}
    </span>
  );
}
